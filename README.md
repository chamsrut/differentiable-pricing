...
| American price-model development loop (task 9H) | **Active — adaptive exploratory development; infrastructure implemented and hardened; one attempt recorded (`scratch_direct_control_v1`, criterion not met).** `train`/`validation` only, price only; selects against validation, so it produces **no project result** | [task spec](docs/tasks/active/task-9h-american-pricer-development.md), [attempt log](docs/attempts/task-9h-attempt-log.jsonl), [decision log](docs/decision-log.md) DEC-041, DEC-042 |
